...
class TestLocation(object):
    ...
    def test_send_live_location(self, bot, chat_id):
        message = bot.send_location(chat_id=chat_id,
                                    latitude=52.223880,
                                    longitude=5.166146,
                                    live_period=80)
        ...
        message2 = bot.edit_message_live_location(message.chat_id,
                                                  message.message_id,
                                                  latitude=52.223098,
                                                  longitude=5.164306)
        ...
        with pytest.raises(BadRequest, match="Message can't be edited"):
            bot.edit_message_live_location(message.chat_id,
                                           message.message_id,
                                           latitude=52.223880,
                                           longitude=5.164306)
    ...
    def test_edit_location_with_all_args(self, bot, location):
        with pytest.raises(ValueError, match='Not both'):
            bot.edit_message_live_location(chat_id=1,
                                           message_id=7,
                                           latitude=2.5,
                                           longitude=4.6,
                                           location=location)
